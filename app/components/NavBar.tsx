...
export default function NavBar({ actions }: { actions?: ReactNode }) {
  ...
  return (
    <header className="sticky top-0 z-20 border-b border-white/[0.06] bg-[#161b27]/90 backdrop-blur-xl">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={logout} disabled={signingOut}
            className="flex items-center gap-1.5 text-xs text-zinc-500 hover:text-zinc-300 px-3 py-1.5 rounded-lg border border-white/[0.06] hover:border-white/[0.12] transition-colors disabled:opacity-60 disabled:cursor-not-allowed">
            {signingOut ? (
              <>
